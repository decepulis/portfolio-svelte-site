...
<div role="tablist" aria-orientation="horizontal" class="relative z-10 inline-flex gap-2">
	{#each tabs as { id, label }, idx (id)}
		...
		<button
			...
			class="hocus:bg-white dark:hocus:bg-gainsboro cursor-pointer border px-4 py-2 font-sans text-sm -outline-offset-4 focus:outline-2"
			class:border-[silver_silver_white_silver]={isActive}
			class:dark:border-[gray_gray_gainsboro_gray]={isActive}
			class:border-silver={!isActive}
			class:dark:border-gray={!isActive}
			class:bg-gainsboro={!isActive}
			class:dark:bg-silver={!isActive}
...
<div
	class="border-silver dark:border-gray relative -mt-[1px] w-full max-w-prose overflow-hidden border transition-[height] duration-300 motion-reduce:duration-0"
